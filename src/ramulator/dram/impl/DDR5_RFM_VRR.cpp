 ...
 * Source:       python/ramulator/dram/ddr5_rfm_vrr.py
 *
...
class DDR5_RFM_VRR : public DRAMSpec {
 public:
  struct Level {
    enum : int { Channel, Rank, BankGroup, Bank, Row, Column, COUNT };
  };
  struct Command {
    enum : int { ACT, PREpb, PREab, RD, WR, RDA, WRA, REFab, RFMab, RFMpb, VRR, COUNT };
  };
  struct State {
...
// Self-registration
static bool _dram_ddr5_rfm_vrr = DRAMSpec::register_standard(
    "DDR5_RFM_VRR", [](const ConfigNode& config) { return std::make_unique<DDR5_RFM_VRR>(config); });
